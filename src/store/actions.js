...
export default {
  ...
  addTask: ({
    ...
    return Task.add(state.auth.token, {
        listId,
        name
      })
      .then((task) => {
  ...
  removeTask: ({
    ...
    return Task.remove(state.auth.token, {
        id,
        listId
      })
      .then(() => {
  ...
  performTaskMoving: ({
    ...
    return Task.move(state.auth.token, {
        id: target,
        from,
        to
      })
      .then(() => {
        commit(types.MOVE_TASK_DONE, {
          target,
          from,
          to
        })
